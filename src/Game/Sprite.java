...
public class Sprite {
    public static Image ASTEROID, GALAXY,SHIP,BULLET;
    static {
        try {
            ASTEROID = ImageManager.loadImage("asteroid");
            GALAXY = ImageManager.loadImage("galaxy");
            SHIP = ImageManager.loadImage("ship1");
            BULLET = ImageManager.loadImage("missile");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
